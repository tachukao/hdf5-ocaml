...
let default_deflate, set_default_deflate = default_get_set 6
...
let hid = function Dataset d -> d | File f -> f | Group g -> g

type open_ = ?meta_block_size:int -> ?split:bool -> string -> t

let open_
    (open_ : string -> ?fapl:Hid.t -> H5f.Acc.t list -> Hid.t)
    acc
    ?meta_block_size
    ?(split = default_split ())
    name =
  ...
  then (
    ...
    if Sys.file_exists (name ^ "-m.h5")
    then
      let command = "cat " ^ name ^ "-m.h5 > /dev/null &" in
      if Sys.command command <> 0
      then failwith (Printf.sprintf "Error executing command \"%s\"" command) );
...
let open_dataset t name = Dataset (H5d.open_ (hid t) (escape name))
let close = function Dataset d -> H5d.close d | File f -> H5f.close f | Group g -> H5g.close g
...
let rec merge ~src ~dst =
  let _ =
    H5l.iterate
      src
      H5_raw.Index.NAME
      H5_raw.Iter_order.NATIVE
      (fun _ name _ () ->
        if H5l.exists dst name
        then (
          let src = H5o.open_ src name in
          let dst = H5o.open_ dst name in
          (match H5i.get_type src, H5i.get_type dst with
          | t, t' when t <> t' ->
            invalid_arg
              (Printf.sprintf "Object %s not of the same type in source and destination" name)
          | H5i.Type.FILE, _ | H5i.Type.GROUP, _ -> merge ~src ~dst
          | H5i.Type.DATASET, _ -> ()
          | H5i.Type.DATATYPE, _
          ...
          | H5i.Type.NTYPES, _ ->
            invalid_arg
              (Printf.sprintf
                 "Unhandled object type %d for the object %s"
                 (Obj.magic (H5i.get_type src))
                 name));
          H5o.close src;
          H5o.close dst )
        else H5o.copy src name dst name;
        H5_raw.Iter.CONT )
      ()
  in
  ()
...
let read_data read expected_datatype create verify t data ?xfer_plist name =
  ...
  if not (H5t.equal expected_datatype datatype) then invalid_arg "Unexpected datatype";
  let dataspace = H5d.get_space dataset in
  let dims, _ = H5s.get_simple_extent_dims dataspace in
  let data = match data with Some data -> verify data dims | None -> create dims in
...
module Make_float (F : Float_arg) = struct
  ...
  let read_float_genarray t ?data name layout =
    read_data
      ...
      (fun data dims ->
        if Genarray.dims data <> dims
        then invalid_arg "The provided storage not of adequate size and dimensions";
        data )
      t
      data
      name
  ...
  let read_float_array t ?data name =
    read_data
      ...
      (fun data dims ->
        if Array.length dims <> 1 then invalid_arg "Dataset not one dimensional";
        if Array.length data < dims.(0) then invalid_arg "The provided data storage too small";
        data )
      t
      data
      name
  ...
  let read_float_array2 t ?data name layout =
    read_data
      ...
      (fun data dims ->
        if Array.length dims <> 2 then invalid_arg "Dataset not two dimensional";
        if Array2.dim1 data < dims.(0) then invalid_arg "The provided data storage too small";
        if Array2.dim2 data <> dims.(1) then invalid_arg "Dim1 of the provided data has wrong size";
  ...
  let read_float_array3 t ?data name layout =
    read_data
      ...
      (fun data dims ->
        if Array.length dims <> 3 then invalid_arg "Dataset not three dimensional";
        if Array3.dim1 data < dims.(0) then invalid_arg "The provided data storage too small";
        if Array3.dim2 data <> dims.(1) then invalid_arg "Dim2 of the provided data has wrong size";
        if Array3.dim3 data <> dims.(2) then invalid_arg "Dim3 of the provided data has wrong size";
    ...
    write_float_array2
      ...
      then (
        ...
        for i = 0 to dim1 - 1 do
          ...
          if Obj.tag e <> Obj.double_array_tag then invalid_arg "All elements not float array";
