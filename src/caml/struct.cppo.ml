...
module Ext = struct
  ...
  let get_string (t : t) pos len =
    let rec index t c l len =
      if l >= len
      then len
      else if String.unsafe_get (Obj.magic t) l = c
      then l
...
module Ptr = struct
  ...
  let seek_float64 t bsize bfield ~min ~max v =
    ...
    while !max > !min + 1 do
      ...
      if v' < v then min := !mid else max := !mid
    done;
  ...
  let seek_float64 t bsize bfield v =
    ...
    if v' < v
    then (
      while !max < len && Ext.get_float64 data (!max * bsize) < v do
        max := !max + !step;
        step := !step * 2
      done;
...
module Make (S : S) = struct
  ...
  module Array = struct
    ...
    let read h5 ?data name =
      ...
      let compound_type = compound_type () in
      if not (H5t.equal compound_type datatype) then invalid_arg "Unexpected datatype";
  ...
  module Vector = struct
    ...
    let create ?(capacity = 16) ?(growth_factor = 1.5) () =
      if growth_factor < 1.
      then invalid_arg (Printf.sprintf "Invalid growth factor %f" growth_factor);
    ...
    let append t =
      ...
      then realloc t (int_of_float (float t.capacity *. t.growth_factor) + 1);
  ...
  module Queue = struct
    ...
    let add t =
      ...
      then hd
        ...
        if pos = 0
        then Array1.blit a (Array1.sub t.a 0 bcapacity)
        else (
