...
let () =
  let data =
    Record.Array.init _LEN (fun i e ->
        Record.set e ~f64:(float_of_int i) ~i ~i64:(Int64.of_int i) ~s:(string_of_int i)
    )
  in
  ...
  Record.Array.iteri data ~f:(fun i e ->
      let i =
        if i < _LEN / 2 then i else if i < (_LEN / 2) + _LEN then i - (_LEN / 2) else i
      in
      assert (Record.f64 e = float_of_int i);
      assert (Record.i e = i);
      assert (Record.i64 e = Int64.of_int i);
