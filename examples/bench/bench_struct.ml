...
let bench s loops f =
  ...
  Printf.printf
    ...
    s
    (if dt > 1.
    then Printf.sprintf "%7.3fs" dt
    else if dt > 0.001
    then Printf.sprintf "%7.3fms" (dt *. 1000.)
    else if dt > 0.000001
    then Printf.sprintf "%7.3fus" (dt *. 1_000_000.)
...
let () =
  ...
  bench "init" len (fun () ->
      let _ =
        Record.Array.init len (fun i r ->
            Record.set r ~a:0. ~b:0. ~c:0. ~d:0. ~e:i ~f:0 ~g:0 ~h:0 ~i:0.)
      in
      ())
