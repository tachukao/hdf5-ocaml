...
let rec op_func loc_id name _info od =
  ...
  (match infobuf.H5o.Info.type_ with
  | H5o.Type.GROUP ->
    Printf.printf "Group: %s {\n%!" name;
    if group_check od infobuf.H5o.Info.addr
    then Printf.printf "%*s  Warning: Loop detected!\n%!" spaces ""
    else (
      let nextod =
        { Opdata.recurs = od.Opdata.recurs + 1
        ; prev = Some od
        ; addr = infobuf.H5o.Info.addr
        }
      in
      return_val :=
        H5l.iterate_by_name
          ...
          nextod);
    Printf.printf "%*s}\n%!" spaces ""
  | H5o.Type.DATASET -> Printf.printf "Dataset: %s\n%!" name
  | H5o.Type.NAMED_DATATYPE -> Printf.printf "Datatype: %s\n%!" name
  | H5o.Type.NTYPES -> Printf.printf "Unknown: %s\n%!" name);
  !return_val
