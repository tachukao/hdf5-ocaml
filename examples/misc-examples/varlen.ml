...
let () =
  ...
  if H5t.get_class tid1 <> H5t.Class.STRING
  then failwith "this is not a variable length string type!!!";
  ...
  if not (H5t.equal native_type tid1) then failwith "native type is not tid1!!!";
