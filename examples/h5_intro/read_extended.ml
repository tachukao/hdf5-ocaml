...
let () =
  ...
  Printf.printf
    "dataset rank %d, dimensions %d x %d\n"
    (Array.length dims)
    dims.(0)
    dims.(1);
  ...
  Printf.printf
    "chunk rank %d, dimensions %d x %d\n"
    (Array.length chunk_dims)
    chunk_dims.(0)
    chunk_dims.(1);
  ...
  H5d.read_bigarray dataset H5t.native_int memspace filespace (genarray_of_array1 column);
  Printf.printf "\n";
