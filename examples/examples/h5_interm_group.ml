...
let () =
  ...
  if H5l.exists file "/G1" then Printf.printf "Group /G1 exists in the file\n";
  let g1_id = H5g.open_ file "/G1" in
  if not (H5l.exists h1_id "G2")
  then (
  ...
  then (
    ...
    for i = 0 to g2_info.H5g.Info.nlinks - 1 do
      let name =
        H5l.get_name_by_idx g2_id "." H5_raw.Index.NAME H5_raw.Iter_order.NATIVE i
      in
